...
New portrait instances are anchored to a visible-image baseline at the camera/canvas bottom-center. Novelify measures the composed portrait layers (including their child offsets, aspect fitting, and transparent sprite bounds) and places the horizontal center of their lowest visible edge on the camera's bottom line; it does not center the portrait's root RectTransform or raw sprite rectangle. A character therefore enters on the floor at every aspect ratio, while explicit normalized positions still map `(-1,-1)` to the lower-left baseline and `(1,1)` to the upper-right baseline. Relative normalized transforms remain displacements rather than absolute screen positions, and both the tween composer and speech-bubble preview use the same baseline convention.
...
**Transform Speaker Portrait** starts with two character target groups. Use **+ Add character** under **Animated Characters** to append as many independently positioned Character/Reference/Position/Rotation/Scale/Margin/Opacity groups as the scene needs. Every target begins on the same frame and shares the node's duration, easing, coordinate space, relative mode, and wait behavior; **- Remove last** removes only the final group.

Double-click **Transform Characters** to open the visual tween composer as a normal resizable Unity editor window; its native title bar provides the standard maximize and close controls. Every connected character is rendered on the same stage and previews simultaneously. Use **Character Target > Editing** to select which portrait the transform frame and numeric controls modify; **Confirm Tween** saves every target back to its own numbered ports. The framed stage automatically follows the currently selected Game View resolution and displays the exact size, normalized screen limits, pixel coordinates, center grid and an optional safe-area guide. Each translucent **START** ghost comes from the matching live character in Play Mode, or from the closest earlier Show/Transform node when editing; chains of earlier relative Transform nodes are accumulated. When a target still has its untouched default transform, its local **TARGET** begins at that incoming START pose. This visual initialization does not modify the node until **Confirm Tween** is clicked, and connected values remain authoritative. The ghosts remain visible while composing or scrubbing and are hidden only while preview playback is active or paused. Drag the selected solid TARGET to move it, drag a corner of its transform frame to scale it, or drag any side of the frame to rotate it. While moving, Shift locks movement to the dominant axis. While scaling, Shift scales from the center and Ctrl makes the scale uniform; both modifiers can be combined. While rotating, Ctrl snaps to 10-degree increments. Arrow keys still provide precise nudging.
...
- **Transform Second Character:** reveals the `Character 2`, `Position 2`, `Rotation 2`, `Scale 2`, `Margin 2`, and `Opacity 2` inputs. Both targets start on the same frame, use the same duration/easing/coordinate space, and Wait For Completion waits for both.
...
While a character is speaking, Novelify can darken every other visible portrait across all of its image layers. Configure this on `NovelGraphRunner` with **Dim Inactive Characters**, **Inactive Character Tint**, and **Speaker Focus Transition Duration**. Lines without a character restore the cast to its normal tint.

### Dialogue and Speech-Bubble Styling

Create a reusable style with **Assets > Create > Novelify > Presentation Style**. The asset contains only the dialogue-box, speaker-name-box, and speech-bubble appearance (fill, opacity, corner radius, and outline); it does not contain text, character, placement, sizing, or timing data. Assign it through a presentation node's **Style Asset** option. When assigned, the matching asset style overrides that node's inline style. Double-click the asset for a three-surface preview and editor. The Dialogue/Speaker and Speech Bubble composers also edit the referenced asset directly, so their preview always matches runtime.

Double-click a **Create Dialogue Box**, **Create Dialogue Speaker Box**, **Create Speech Bubble Box**, or **Change Speech Bubble Box** node to open its visual composer. A story **Speech Bubble** is a dialogue node: it owns the line's Dialogue text and character target, and double-clicking it does not open the box-style composer.

Connect **Preview Character** or **Preview Character Reference** on a Speech Bubble Box node before opening its composer. The composer walks the incoming story path for that exact instance and places the portrait at its latest Show/Transform position, rotation, and scale, so the tail and offsets are composed against the character's authored on-screen location. **Preview Instance ID** selects a non-default copy when a plain Character input is used.

Story **Speech Bubble** nodes expose explicit **Character** and **Character Reference** inputs. These are the authoritative target for runtime following and context preview; the older Speaker inputs remain a compatibility fallback. An unwired bubble shows no character in the preview instead of selecting an unrelated graph character.
