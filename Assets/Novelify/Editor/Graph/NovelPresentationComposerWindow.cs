...
namespace Novelify.Editor
{
    ...
    internal sealed class NovelPresentationComposerWindow : EditorWindow
    {
        private sealed class PresentationDraft
        {
            ...
            //The resolution must be ints, so we dont use a normal vector2 field
            ...
            CreateDialogueSpeakerBoxNode speaker)
        {
            ...
            _speakerNodes.Clear();
            _dialogueNodes.AddRange(
                graph.GetNodes().OfType<CreateDialogueBoxNode>());
            _speakerNodes.AddRange(
                graph.GetNodes().OfType<CreateDialogueSpeakerBoxNode>());
            _dialogueIndex = dialogue != null
                ? Mathf.Max(0, _dialogueNodes.IndexOf(dialogue))
                : 0;
            _speakerIndex = speaker != null
                ? Mathf.Max(0, _speakerNodes.IndexOf(speaker))
                : 0;
        ...
        {
            string sampleSpeaker = _draft?.SampleSpeaker ?? "Test Speaker";
            string sampleDialogue = _draft?.SampleDialogue ??
                "This is sample dialogue for layout preview.";
            Vector2Int resolution = _draft?.Resolution ??
        ...
        private void Rebuild()
        {
            ...
            rootVisualElement.style.backgroundColor =
                (Color)new Color32(18, 27, 43, 255);
            rootVisualElement.style.color =
                (Color)new Color32(226, 232, 240, 255);
        ...
        private VisualElement BuildToolbar()
        {
            ...
            toolbar.style.backgroundColor =
                (Color)new Color32(28, 40, 59, 255);
            ...
            {
                PopupField<string> dialogue = new PopupField<string>(
                    "Dialogue node",
                    NodeNames("Dialogue Box", _dialogueNodes.Count),
                    Mathf.Clamp(_dialogueIndex, 0, _dialogueNodes.Count - 1));
                dialogue.style.width = 205f;
                dialogue.RegisterValueChangedCallback(_ =>
                    SelectDialogue(dialogue.index));
                toolbar.Add(dialogue);
            }
            if (_speakerNodes.Count > 0)
            {
                PopupField<string> speaker = new PopupField<string>(
                    "Speaker node",
                    NodeNames("Speaker Box", _speakerNodes.Count),
                    Mathf.Clamp(_speakerIndex, 0, _speakerNodes.Count - 1));
                speaker.style.width = 205f;
                speaker.RegisterValueChangedCallback(_ =>
            ...
            if (_draft != null)
            {
                int resolutionIndex = ResolutionIndex(_draft.Resolution);
                DropdownField resolution = new DropdownField(
                    "Resolution",
                    ResolutionNames.ToList(),
                    resolutionIndex);
                resolution.style.width = 185f;
                resolution.RegisterValueChangedCallback(_ =>
                {
                    _draft.Resolution = ResolutionAt(resolution.index);
                    RefreshPreview();
                });
        ...
        private VisualElement BuildPreviewHost()
        {
            ...
            Label badge = new Label("EDITOR PREVIEW");
        ...
        private VisualElement BuildInspector()
        {
            ...
            inspector.style.backgroundColor =
                (Color)new Color32(24, 35, 52, 255);
            Foldout samples = NewFoldout("Preview Content");
            AddText(samples, "Speaker Name", _draft.SampleSpeaker, false,
                value => _draft.SampleSpeaker = value);
            AddText(samples, "Dialogue", _draft.SampleDialogue, true,
                value => _draft.SampleDialogue = value);
        ...
        private VisualElement BuildFooter()
        {
            ...
            footer.style.paddingTop = 7f;
            footer.style.paddingBottom = 7f;
            footer.style.backgroundColor =
                (Color)new Color32(28, 40, 59, 255);
            _status = new Label("Previewing saved graph values.");
            _status.style.flexGrow = 1f;
            _status.style.color =
                (Color)new Color32(148, 163, 184, 255);
            footer.Add(_status);
            Button revert = new Button(Revert) { text = "Revert" };
            ...
            NovelBoxStyle defaults)
        {
            ...
            Button reset = new Button(() =>
            {
                ...
                _draftDirty = true;
                Rebuild();
                SetStatus("Style reset in the draft. Apply to save.", false);
            })
            { text = "Reset Style" };
        ...
        private void DraftChanged()
        {
            ...
            _draftDirty = true;
            RefreshPreview();
            SetStatus("Draft changed. Apply to save it to the graph.", false);
        }
        ...
        private void ApplyToGraph()
        {
            if (_graph == null || DialogueNode == null || SpeakerNode == null)
                return;
            ValidateDraft();
            _graph.UndoBeginRecordGraph("Compose Dialogue Presentation");
            WriteDialogueOptions(DialogueNode, _draft.Dialogue);
            WriteSpeakerOptions(SpeakerNode, _draft.Speaker);
            _draftDirty = false;
            SetStatus("Dialogue and speaker presentation saved.", false);
        }
        private void Revert()
        {
            if (!_draftDirty || EditorUtility.DisplayDialog(
                    "Revert presentation draft?",
                    "Discard the unapplied changes in this composer?",
                    "Revert",
                    "Keep Editing"))
            {
                LoadDraft();
                Rebuild();
                SetStatus("Draft restored from the graph.", false);
            }
        }
        ...
        private static int ResolutionIndex(Vector2Int value)
        {
            for (int index = 0; index < ResolutionNames.Length; index++)
                if (ResolutionAt(index) == value) return index;
            return 0;
        }
        private static Vector2Int ResolutionAt(int index)
        {
            return index switch
            {
                1 => new Vector2Int(1600, 900),
                2 => new Vector2Int(1280, 720),
                3 => new Vector2Int(1440, 900),
                4 => new Vector2Int(1024, 768),
                5 => new Vector2Int(1080, 1920),
